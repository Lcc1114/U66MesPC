...
namespace U66MesPC.Common.Station
{
    ...
    /// <summary>
    /// 对应工站：PCBA机台下料，所在模组：PCBA attach to cap
    /// </summary>
    public class PCBAUnLoad : StationBase
    {
        ...
        public PCBAUnLoad(SysConfigs sysConfig) : base(sysConfig)
        {
        ...
        public bool ClipSNCheck(int station)
        {
            try
            {
                ...
                AddWriteLogInfo(Configs.StationID, "ToPLC", $"{station}#ClipCheck result:{result};弹夹：{clipID}");
                return result;
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                ...
                //AddWriteLogInfo(Configs.StationID, "ToPLC", $"Reset ProductSNCheck flag");
            }
        }
        /// <summary>
        /// 核对载具的合法性
        ...
        public bool InnerCheckIn()
        {
            try
            {
                ...
                AddWriteLogInfo(Configs.StationID, "ToPLC", $"CheckIn({carrierID}): result:{ret}");
                if (ret)
                {
        ...
        {
            AddWriteLogInfo(Configs.StationID, "FromPLC", $"{station}#弹夹出站请求");
