...
namespace U66MesPC.Common.Station
{
    ...
    /// <summary>
    /// 对应工站：E-Flex机台上料，所在模组：E-Flex Attach to Cap&PCBA
    /// </summary>
    public class EFlexLoad : StationBase
    {
        ...
        public EFlexLoad(SysConfigs sysConfig) : base(sysConfig)
        {
        ...
        public bool InnerCheckIn()
        {
            try
            {
                ...
                AddWriteLogInfo(Configs.StationID, "写入PLC", $"CheckIn result:{ret}；CarrierID:{carrierID}");
                if (ret)
                {
        ...
        public override bool CheckOut()
        {
            try
            {
                ...
                {
                    throw new ArgumentErrorException($"出站失败：载具码（{carrierID}）或产品码（{ info.Value?.Count()}个）为空");
                }
                ...
                if (bSimulation)
                    ret = true;
                //AddWriteLogInfo(Configs.StationID, "写入PLC", $"CheckOut：{ret}");
